...
private:
    static bool file_exists (string_type const & p) {
#if defined(_WIN32) || defined(_WIN64)
...
#   error "Need to implement for Windows"
    ...
    bool open (string_type const & p, std::error_code & ec)
    {
#if defined(_WIN32) || defined(_WIN64)
#   error "Need to implement for Windows"
#else // POSIX
...
#   error "Need to implement for Windows"
    ...
    string_type const & path () const noexcept
    {
        return _path;
    }
...
private:
    ...
    string_type _path;   // contains path to dynamic library
};
